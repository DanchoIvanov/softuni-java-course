...
public class FilterByAge {
    public static void main(String[] args) {
        ...
        people.stream()
                .filter(getPredicate(criteria, age))
                .forEach(printer(format));
    }
    public static Predicate<Person> getPredicate (String criteria, int age){
        return criteria.equals("older")
                ? p -> p.age >= age
                : p -> p.age <= age;

    }

    public static Consumer<Person> printer (String[] format) {
        if (format.length == 2){
