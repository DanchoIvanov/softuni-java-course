...
//import static java.awt.font.StyledParagraph.insertInto;
...
public class  BinarySearchTree<E extends Comparable<E>> {
    ...
    public void insert(E element) {
        if (this.root == null) {
            ...
            insertInto(this.getRoot(), element);
        }
    }
    ...
    public BinarySearchTree<E> search(E element) {
        ...
        while (current != null) {
            if (current.getLeft() != null && isLess(element, current)) {
                current = current.getLeft();
            } else if (current.getRight() != null && isLess(element, current)) {
    ...
    public void deleteMax() {
        ...
        current.count++;
        current.leftChild = current.getRight().getLeft();
    ...
    private boolean isEqual(E first, Node<E> second) {
        return first.compareTo(second.getValue()) == 0;
    }
